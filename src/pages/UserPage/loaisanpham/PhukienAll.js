...
function PhukienAll({ match }) {
  const listKhoQuanAo = useSelector(state => state.quanlyKhoAoQuan.list);
  const listPhuKien = listKhoQuanAo.filter(item => item.loaisanphamId === 6);
  ...
  function renderTatCaPhukien(listPhuKien) {
  ...
  let listSanPhamTshirtTangDan = _.orderBy(listPhuKien, ['price'], ['asc']);
  let listSanPhamTshirtGiamDan = _.orderBy(listPhuKien, ['price'], ['desc']);
  let listSanPhamTshirtMoiNhat = listPhuKien.filter(item => item.sanphamduocId === 6);
  let listSanPhamTshirtCuNhat = listPhuKien.filter(item => item.sanphamduocId === 5);
        ...
        <div className="container">
          <div className="row">
            <div className="col-xs-12 a-left">
              <p> Trang chủ &ensp;  <i class="fa fa-chevron-right" aria-hidden="true"></i> &ensp;  <span>PHỤ KIỆN</span></p>
            </div>
            <div className="main_container collection col-md-12 col-lg-12">
              ...
              <div className="row renderProductAll">
                {renderTatCaPhukien(listPhuKien)}
              </div>
              <Row justify="center">
                {lengthRender !== listPhuKien.length && listPhuKien.length > 0 ?
                  <Button
