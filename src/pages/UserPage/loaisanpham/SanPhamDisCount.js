...
function SanPhamDisCount({ match }) {
  const listKhoQuanAo = useSelector(state => state.quanlyKhoAoQuan.list);
  const listSanPhamKhuyenMai = listKhoQuanAo.filter(item => item.sanphamduocId === 3 );
  console.log(listSanPhamKhuyenMai);
  ...
  function renderSanPhamKhuyenMai(listSanPhamKhuyenMai) {
    if (renderProduct !== null && renderProduct !== undefined) {
      ...
      return (
        listSanPhamKhuyenMai.map((item, index) => {
          if (index < lengthRender) {
            return (
              <div className="col-xl-3 col-lg-3 col-6">
                <div className="card-best-seller">
  ...
  let listSanPhamTshirtTangDan = _.orderBy(listSanPhamKhuyenMai, ['priceSale'], ['asc']);
  let listSanPhamTshirtGiamDan = _.orderBy(listSanPhamKhuyenMai, ['priceSale'], ['desc']);
  let listSanPhamTshirtMoiNhat = listSanPhamKhuyenMai.filter(item => item.sanphamduocId === 6);
  let listSanPhamTshirtCuNhat = listSanPhamKhuyenMai.filter(item => item.sanphamduocId === 5);
        ...
        <div className="container">
          <div className="row">
            <div className="col-xs-12 a-left">
              <p> Trang chủ &ensp;  <i class="fa fa-chevron-right" aria-hidden="true"></i> &ensp;  <span>SẢN PHẨM KHUYỄN MÃI</span></p>
            </div>
            <div className="main_container collection col-md-12 col-lg-12">
              <SlickImage />
              <div className="tatcasanpham-title">
                <strong>SẢN PHẨM KHUYỄN MÃI</strong>
              </div>
              ...
              <div className="row renderProductAll">
                {renderSanPhamKhuyenMai(listSanPhamKhuyenMai)}
              </div>
              {/* <Row justify="center">
                {lengthRender === listSanPhamKhuyenMai.length ? <> </> :
                  <Button
...
export default SanPhamDisCount;
