...
function HoodieNewArrival({ newArrival, match }) {
  ...
  const newArrivalHoodie = newArrival.filter((itemHoodie) => itemHoodie.loaisanphamId === 3);
  function renderHoodieAll() {
    return newArrivalHoodie.filter((itemMaSanPham) => itemMaSanPham.maSanPham !== "YG21-NEWARRIVAL1-S1HOODIE-Đ").length > 0
      ? newArrivalHoodie.filter((itemMaSanPham) => itemMaSanPham.maSanPham !== "YG21-NEWARRIVAL1-S1HOODIE-Đ").map((item, index) => {
        return (
          <>
            <Space >
              <Card bordered={false}
...
export default HoodieNewArrival;